...
static functio i_busca_tipo
   PROD->(Dbgotop())

   While ! PROD->(Eof())
      if MATERIAL->(Dbseek(val(right(PROD->Codigo,5)))) .and. PROD->(qrlock())
         if MATERIAL->Mat_Catego == 0
            replace PROD->Consumivel with "C"
         else
            replace PROD->Consumivel with "P"
         endif
         PROD->(Qunlock())
      endif
