...
static function i_inicializacao
   ...
   ENT_ACA->(dbsetorder(1))
   ENT_ACA->(dbgotop())
   set softseek on
   ENT_ACA->(dbseek(dtos(cDT_INI)))
   set softseek off
...
static function i_imprime
   ...
   do while ! ENT_ACA->(eof()) .and. ENT_ACA->Data >= cDT_INI .and. ENT_ACA->data <= cDT_FIM
      ...
      if XPAGINA = 0 .or. prow() > 57
         ...
         @ prow()+2,0 say "Data        Produto                                         Lote            Quant    Obs"
         @ prow()+1,0 say ""

      endif

      @ prow()+1,00 say dtoc(ENT_ACA->Data)
      PROD->(dbseek(ENT_ACA->Cod_prod))
      @ prow()  ,12 say left(PROD->Descricao,45)
      @ prow()  ,60 say ENT_ACA->Lote
      @ prow()  ,75 say transform(ENT_ACA->Quantidade,"@R 999999")
      @ prow()  ,85 say left(ENT_ACA->Obs,40)

      nQUANT += ENT_ACA->Quantidade
   ...
   @ prow()+1,0 say " TOTAIS..."
   @ prow()  ,73 say transform(nQUANT,"@R 99999999")
   @ prow()+1,0 say replicate("-",132)
