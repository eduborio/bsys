...
MOVIMENT->(dbSetFilter({|| Tipo == "B".and. Contabil == .F. }))
...
MOVIMENT->(qview({{"Data/Data"                    ,3},;
                  ...
                  {"transform(Val_uni, '@R 999,999.99')/Valor Unt rio"  ,0}},"P",;
                  {NIL,"c104c",NIL,NIL},;
...
function c104b
  PROD->(dbseek(MOVIMENT->(Cod_Prod)))
return left(PROD->Descricao,20) + left(PROD->Marca,12)
...
static function i_edicao
   ...
   if cOPCAO <> "I"
      ...
      qrsay ( XNIVEL++ , transform(MOVIMENT->Val_uni, "@e 999,999.99"))
   ...
   if MOVIMENT->(iif(cOPCAO=="I",qappend(),qrlock()))
      ...
      fTIPO := "B"     // para determinar que a operacao ‚ de baixa
      fCONTABIL := .F. // baixa sem contabilizacao
    ...
    PROD->(Dbsetorder(4))
    INVENT->(Dbsetorder(1))
    if INVENT->(Dbseek(fFILIAL+fCOD_PROD+fLOTE)) .and. INVENT->(qrlock()) .and. MOVIMENT->(qrlock())
