...
MOV_PROJ->(dbSetFilter({|| Tipo == "EC" }, 'Tipo == "EC" '))
...
static function i_edicao
   ...
   if MOV_PROJ->(iif(cOPCAO=="I",qappend(),qrlock()))
      ...
      fTIPO  := "EC"      //ENTRADA de Estorno de Consumiveis/Sobra
      fDATA  := cDATA
      fFILIAL:= cFILIAL
...
static function i_exclusao
   if qconf("Confirma exclus„o desta Entrada de Estorno/Sobra ?")
      if MOV_PROJ->(qrlock())
         PROD->(Dbsetorder(4))
         INVENT->(Dbsetorder(4))
