...
// ALTERACAO FEITA EXCLUSIMENTE P/SIDELMA CAMOES D.S.R. E PRODUTIVIDADE  //EDUARDO AUGUSTO BORIO
// SALARIO CONTRATUAL
                if SITUA->Salario <> 0
                   bVALOR := 0
                   LANC->(dbgotop())
                   LANC->(dbseek("199910"+FUN->Matricula))
                   do while LANC->Anomes == "199910" .and. LANC->Matricula == FUN->Matricula
                       do case
                          case LANC->Evento == "149"
                               bVALOR += LANC->Valor

                          case LANC->Evento == "153"
                               bVALOR += LANC->Valor
                       end case
                       LANC->(dbskip())
                   enddo

                   bVALOR += SITUA->Salario
                   cRAIS += cTIPO + strzero(val(qtiraponto(str(bVALOR,9,2))),9)
                   bVALOR := 0
                Else
                   BASE->(dbgotop())
                   BASE->(dbsetorder(1))
                   if BASE->(dbseek(FUN->Matricula+"199912"))
                      cRAIS += cTIPO + strzero(val(qtiraponto(str(BASE->B_FGTSMS,9,2))),9)
                   else
                      BASE->(dbseek(FUN->Matricula+"199910"))
                      cRAIS += cTIPO + strzero(val(qtiraponto(str(BASE->B_FGTSMS,9,2))),9)
                   endif
                endif
