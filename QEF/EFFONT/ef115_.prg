...
// OBJETIVO...: MANUTENCAO DE TABELA DE REGIME SIMPLIFICADO - CRECHES
...
static function i_edicao
   ...
   for nCONT := 1 to 12
       if nCONT != 1
          qsay(nCONT+9,27,nLIMITE+0.01,"@E 9,999,999.99")
       endif
       cMACRO   := "fRS_LIM" + strzero(nCONT,2)
       zTMP := &cMACRO
       qmensa("Digite 0 para finalizar...")
       qgetx(nCONT+9,44,@zTMP,"@K@E 9,999,999.99")
       ...
       if nLIMITE == 0
          nLIMITE := 9999999.99
          &cMACRO := 9999999.99
          qsay(nCONT+9,44,nLIMITE,"@E 9,999,999.99")
       endif

       cMACRO   := "fRS_ALI" + strzero(nCONT,2)
       zTMP := &cMACRO
       qmensa("Digite a aliquota...")
       qgetx(nCONT+9,62,@zTMP,"@E 99.99")
...
function f115b
   ...
   for nCONT := 1 to 12
      ...
      qsay(nCONT+9,27,nLIMITE+0.01,"@E 9,999,999.99")
      nLIMITE := &cMACRO
      qsay(nCONT+9,44,&cMACRO,"@E 9,999,999.99")
      cMACRO  := "RS_ALI" + strzero(nCONT,2)
      qsay(nCONT+9,62,&cMACRO,"@E 99.99")
      cMACRO  := "RS_IPI" + strzero(nCONT,2)
      qsay(nCONT+9,74,&cMACRO,"@E 99.99")
      if nLIMITE >= 9999999
         exit
      endif
   next
