...
private sBLOC1     := qlbloc("B529B","QBLOC.GLO") // Tipo: Valor contabil / Valor de prestacao de servico
...
private nVLR_CONT                                 //Valor Contabil Total do Dia
private nVLR_MERC                                 //Valor das Mercadorias Total do Dia
private nISS_BASE                                 //Base de ISS Total do Dia
private nFOLHA                                    // Numero da Folha
private nIMPOSTO                                  // Total do imposto
private nVAL_TRIB                                 // Valor total do ISS
private cPRIM_NF                                  // Primeira Nota do DIA
private zPRIM_NF                                  // Primeira Nota do DIA
private lPRI_NFD                                  // Primeira Nota do DIA
private cULT_NF                                   // Ultima Nota do Dia
...
static function i_inicializacao
   ...
   ISS->(dbSetFilter({|| ISS->DATA_LANC>= dDATA_INI .and. ISS->DATA_LANC<=dDATA_FIM}, 'ISS->DATA_LANC >= dDATA_INI .and. ISS->DATA_LANC<=dDATA_FIM'))
   ...
   ISS->(dbgotop())
   cPRIM_NF := ISS->Num_nf
   nDIA := val(substr(dtoc(ISS->Data_lanc),1,2))
   select SERV
   SERV->(dbgotop())
...
//       XPAGINA++
      ...
      if eval(bFILTRO)
         ...
         if substr(dtoc(ISS->Data_lanc),1,2) > strzero(nDIA,2)
            lIMP := .T.
         endif

         if substr(dtoc(ISS->Data_lanc),1,2) == strzero(nDIA,2)
            nVLR_CONT += ISS->Vlr_cont
            nVLR_MERC += ISS->Vlr_merc
            nISS_BASE += ISS->Iss_Base
         endif

         if lIMP
                 ISS->(dbskip(-1))

                 @ prow()+1,05  say strzero(nDIA,2) + "   " +;
                 ...
                 do case
                    case cTIPO_LIVRO == "1"
                         @ prow()  ,76 say transform(nVLR_CONT,cPIC1) + "       " +;
                                           transform(nVLR_MERC,cPIC1) + "   " +;
                                           transform(nISS_BASE,cPIC1) + " " +;
                                           left(ISS->OBS,27)


                    case cTIPO_LIVRO == "2"
                         @ prow()  ,76 say transform(nIss_base,cPIC1) + "   " +;
                                           transform(nIss_base,cPIC1) + " " +;
                                           left(ISS->OBS,27)
...
static function i_tot_periodo
   ...
   do case
      case cTIPO_LIVRO == "1"
           ...
           @ prow()+2,126 say "Imposto a recolher:" + space(02) + transform(nIMPOSTO,cPIC1)
      case cTIPO_LIVRO == "2"
           @ prow()+1,05  say replicate("-",127)
           @ prow()+1,85  say "Total: " + transform(nVAL_TRIB,cPIC1)
           @ prow()+2,98  say "Imposto a recolher:" + space(02) + transform(nIMPOSTO,cPIC1)
   endcase
