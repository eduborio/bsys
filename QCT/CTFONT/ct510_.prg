...
private sBLOCO1  := qlbloc("B510B","QBLOC.GLO") // tipo de relatorio
private sBLOCO2  := qlbloc("B510C","QBLOC.GLO") // mensagem de alerta
...
//      endif
      ...
      if PLAN->Reduzido != cREDAPU .or. (PLAN->Reduzido == cREDAPU .and. cTIPO == "2") .and. cNIVEL == "5"
         @ prow()+1,nCOL1 say PLAN->Descricao
   ...
   do while ! LANC->(eof())
      ...
      if ! empty(LANC->Cont_db)
         ...
         if ! PLAN->(eof())
            ...
            ct1_prop_ab(PLAN->Codigo,LANC->Valor,"DBAB")
      ...
      if ! empty(LANC->Cont_cr)
         ...
         if ! PLAN->(eof())
            ...
            ct1_prop_ab(PLAN->Codigo,LANC->Valor,"CRAB")
...
function ct1_prop_ab ( cCONTA , nVALOR , cMACRO )
   local nREC := PLAN->(recno())
   local nORD := PLAN->(indexord())
   local nNEG
   ...
   do case
      case len(cCONTA) == 12 ; ct1_prop_ab ( left(cCONTA,7) , nVALOR , cMACRO )
      case len(cCONTA) == 7  ; ct1_prop_ab ( left(cCONTA,4) , nVALOR , cMACRO )
      case len(cCONTA) == 5  ; ct1_prop_ab ( left(cCONTA,2) , nVALOR , cMACRO )
      case len(cCONTA) == 3  ; ct1_prop_ab ( left(cCONTA,1) , nVALOR , cMACRO )
   endcase
