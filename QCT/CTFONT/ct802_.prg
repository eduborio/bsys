...
RESULT->(qview({{"Titulo/T¡tulo",0},{"i_802b()/Conte£do",0}},"P",;
                {"i_802a",NIL,NIL,NIL},;
...
function i_combinacao
   ...
   private sBLOCO1 := qlbloc("B802A","QBLOC.GLO") // combinacao

   qrbloc(07,14,sBLOCO1)

   PLAN->(dbsetorder(3))

   cCOMBINA := RESULT->Combina2
   ...
   if RESULT->(qrlock())
      replace RESULT->Combina2 with cCOMBINA
      RESULT->(qunlock())
   else
      qm2()
   endif
