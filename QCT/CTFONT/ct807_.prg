...
FLUXO->(qview({{"left(Descricao,60)/Codigo",1},{"i_807b()/Conte£do",0}},"P",;
                {"i_807a",NIL,NIL,NIL},;
...
function i3_combinacao
   ...
   if FLUXO->(qrlock())
      replace FLUXO->Combina2 with cCOMBINA
      FLUXO->(qunlock())
   else
      qm2()
   endif
